...
cancha_image = pygame.image.load("Images/cancha.png")
cancha_image = pygame.transform.scale(cancha_image, (WIDTH, HEIGHT - 100))

# Áreas de gol ajustadas
AREA_GOL_1 = pygame.Rect(0, 252, 40, 170)
AREA_GOL_2 = pygame.Rect(WIDTH - 40, 252, 40, 170)
...
portero_1 = Portero(1, 40, 300, None, aleatorio=True)
portero_2 = Portero(2, WIDTH - 40, 300, None, aleatorio=True)
balon = Balon()
...
def reiniciar_balon():
    ...
    balon.vy = rd.uniform(-5, 5)

def mover_balon():
    balon.x += balon.vx
    balon.y += balon.vy

    if balon.x <= 20 or balon.x >= WIDTH - 20:
        balon.vx *= -1
    if balon.y <= 100 or balon.y >= HEIGHT - 150:
        balon.vy *= -1
...
def dibujar_circulos_tiempo(x, y, minutos_transcurridos):
    ...
    for i in range(5):
        ...
        pygame.draw.circle(screen, color, (x + i * 30, y), 8)
...
def main():
    ...
    while True:
        ...
        # Dibujar cancha
        screen.blit(cancha_image, (0, 60))

        # Dibujar marcador y nombres de equipos
        pygame.draw.rect(screen, ORANGE, (0, 0, WIDTH // 3, 65))  # Fondo Equipo A
        pygame.draw.rect(screen, BLUE, (2 * WIDTH // 3, 0, WIDTH // 3, 65))  # Fondo Equipo B
        pygame.draw.rect(screen, BLACK, (WIDTH // 3, 0, WIDTH // 3, 65))  # Fondo marcador
        ...
        # Posicionar textos
        screen.blit(equipo_a_text, (20, 23))
        screen.blit(equipo_b_text, (WIDTH - 120, 23))
        screen.blit(marcador_text1, (WIDTH // 4 - marcador_text1.get_width() // 2, 15))
        screen.blit(marcador_text2, (WIDTH // 1.35 - marcador_text2.get_width() // 2, 15))
        screen.blit(tiempo_text, (WIDTH // 2 - tiempo_text.get_width() // 2, 10))
        ...
        if en_juego and not pausa:
            tiempo_juego += clock.get_time() / 1000
...
# Areas para prueba de diseño de porterias y del area de la ventana

        pygame.draw.rect(screen, (0, 0, 0), AREA_GOL_1, width=2)
        pygame.draw.rect(screen, (0, 0, 0), AREA_GOL_2, width=2)
        pygame.draw.rect(screen, (0, 0, 0),(30, 78, 740, 515) , width=2)
